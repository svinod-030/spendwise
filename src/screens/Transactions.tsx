    ...
    return (
      <View className="flex-row items-center justify-between bg-slate-900 p-4 rounded-2xl mb-3 border border-slate-800">
        <View className="flex-1">
          ...
          <Text className="text-slate-400 text-xs mt-1">{new Date(item.date).toLocaleString()}</Text>
        </View>
        <View className="items-end">
          <Text className={`font-bold ${display.colorClass}`}>
            {display.sign}${item.amount.toFixed(2)}
          </Text>
          <Text className="text-[10px] text-slate-400 mt-1">{display.label}</Text>
        </View>
      </View>
    );
  };
      ...
      <View className="px-6 pb-3 flex-row flex-wrap">
        {(["all", "expense", "income", "refund", "transfer"] as const).map((filter) => (
          <TouchableOpacity
            ...
            className={`px-3 py-2 rounded-xl mr-2 mb-2 border ${activeFilter === filter ? "bg-blue-500/20 border-blue-400" : "bg-slate-900 border-slate-800"}`}
          >
            <Text className="text-white text-xs capitalize">{filter}</Text>
          </TouchableOpacity>
        ))}
      </View>
